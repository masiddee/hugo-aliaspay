$(function(){
  ...
  $('#signup-merchant-form').on('submit', function(e){
    ...
    fetch(`https://rtc-app.herokuapp.com/subscriptions/merchants/?${query}`, options)
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      if(data.success === true){
        $('.loader').remove();              
        $(this).html('<p class="signup-merchant-success">Thanks for signing up! We will follow up soon with details to complete the beta enrollment process.</p>');
      }
    })
    .catch((err) => {
      $('.loader').remove();
